...
| | ② **首屏 hero 加载优化(B+C)**：①遮罩缩短——首屏初始等待 150→50ms、渐隐 480→300ms(JS 常量与 custom.css 同步);②hero 大图预加载——文章详情(`inject-theme.js` injector `post` 类型注入 post-hero-banner)/ 工程详情 / 工程列表 / 音乐 / 关于 / 瞬间共 6 页 `<link rel=preload as=image>`,遮罩期间图片并行就绪,退场不再"图片后到"突兀;;③版本号 p44→p45 | `source/rose-galaxy/js/nova-ux.js`、`source/css/custom.css`、`scripts/inject-theme.js`、`themes/butterfly/layout/{project-detail,projects,moments,music,about}.pug`、`_config*.yml` |
| | ④ **首屏 hero「图片后到」修复(线上实测定位)**:hero preload 默认低优先级,被 11 个 CSS + 多个 JS 排队(线上实测延迟 ~1.9s)→ 6 页 hero preload 加 `fetchpriority="high"`+`type=image/webp`(与首页 background02 同策略);遮罩关键样式内联进 `nova-loading.html`(body_begin 首帧即渲染,不再等 custom.css 全量加载,消除裸页面窗口);线上复测:hero 648ms 完成 < 遮罩 1490ms 出现 → 正延迟消除;版本 p45→p46 | `scripts/inject-theme.js`、`themes/butterfly/layout/{project-detail,projects,moments,music,about}.pug`、`themes/butterfly/layout/parts-common/nova-loading.html`、`_config*.yml` |
