...
```bash
npm run build                       # 生成 + 压缩 + 冒烟检查，失败非零退出
npm test                            # 68 项单元测试
npm run verify -- --strict          # 18 项：结构断言 + 基线比对
```

`test/baseline.json` 记录关键指标：产物文件数、文章目录数、三个生成物的字节数、首页排序首项。**正常新增文章或工程后**用下面这条刷新基线，其余情况出现基线不符就说明出了问题。

```bash
npm run verify -- --update-baseline
```
...
| `public/` 文件数 | 127 |
| `public/posts/` 目录数 | 15 |
| `search.xml` | 149,662 字节 |
| `sitemap.xml` | 3,256 字节 |
| `atom.xml` | 10,927 字节 |
| 首页 LATEST SIGNAL | `[文章] 绘世 Stable Diffusion @ 2026-09-03` |
| 精选工程顺序 | `led-matrix → drone → line-car` |

> 判据用字节数、条目数与排序首项，**不要用文件哈希**。Hexo 的生成器输出并不确定，不经 minify 的两次全量生成也会有差异，但字节数相同。
