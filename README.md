...
| 深色主题 | 浅色主题 |
| --- | --- |
| ![深色主题预览](source/img/misc/preview-dark.png) | ![浅色主题预览](source/img/misc/preview-light.png) |
...
| 日期 | 改动 | 涉及文件 |
| --- | --- | --- |
| 2026-09-01 | **文案体系定稿 + 新版演示站(重建自主站克隆, 每步git回退)**：①**页面题记统一**(站内可见): 文章索引"文章合为时而著，歌诗合为事而作。"、标签页"文章本天成，妙手偶得之。"、工程页"如果我看得更远，那是因为我站在巨人的肩膀上。"、说说页副标题"记录一些没有主题的日常、情绪与瞬间。"、音乐页加问号;②**分享/搜索描述(og:description)与站内题记职责分离**——描述保持功能信息版(如工程页"单片机与建模的动手实录：从开源复刻到课程设计。"), 不随题记更改(分享卡片由平台抓取, 站内编辑推荐信息版);③说说页样式定稿: 眉标 nova-tag-eyebrow 同色(删除 hero-copy p 覆盖规则)、h1 38-62px/小屏36px、hero 图框 clamp(430px,52vh,610px)、hero 底部 ::after 过渡 8%(transparent→--shuo-bg)、hero-shade 浅色回调版;关于页: h1 2.4-4.2rem、英文副标题 Documenting my learning…、头像+标题并排+简介整行左对齐、卡片顺序 QQ/EMAIL/GITHUB、padding 22-40、副标题"学如逆水行舟";工程页按钮 EXPLORE THE TOPICS;④版本号 bump v=20260831-p1(全站13文件29处, 破除缓存) | `themes/butterfly/layout/*`、`source/rose-galaxy/css/{moments,about,tag}-page.css`、`scripts/projects-generator.js`、`_config.butterfly.yml`、README |
